...
def load_sweperf_dataset(name="princeton-nlp/SWE-bench", split="test", instance_ids=None) -> list[SWEPerfInstance]:
    """
    ...
    if name.endswith(".json"):
        ...
        if Path(name).exists():
            ...
            # SWE-Perf dataset only has 'test' split, use direct parquet loading
            import pandas as pd
            df = pd.read_parquet('hf://datasets/SWE-Perf/SWE-Perf/data/test-00000-of-00001.parquet')
    ...
    for idx in range(len(dataset)):
        dataset[idx]["duration_changes"] = json.loads(dataset[idx]["duration_changes"])
    return dataset
