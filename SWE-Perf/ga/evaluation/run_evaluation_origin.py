...
def run_instance(
    ...
    try:
        ...
        container = build_container(test_spec, client, run_id, logger, rm_image, force_rebuild, is_large = True, from_remote = True)
        container.start()
...
def run_instances(
    ...
    # Change instance_image_key to the remote one
    for i in range(len(test_specs)):
        docker_image_prefix = 'docker.io/betty1202/'
        image_name = 'sweb.eval.x86_64.' + test_specs[i].instance_id
        image_name = image_name.replace(
            '__', '_s_'
        )  # to comply with docker image naming convention
        test_specs[i].instance_image_key = (docker_image_prefix + image_name).lower()
