...
def build_instance_images(
    ...
    Optimized to build only unique repo-version combinations instead of per-instance images.
    This reduces the number of images from 140 to ~35.
    ...
    test_specs = list(map(func, dataset))
    
    # Deduplicate test specs by instance_image_key (repo-version combination)
    # This is the key optimization: build only unique repo-version images
    unique_specs = {}
    for spec in test_specs:
        image_key = spec.instance_image_key
        if image_key not in unique_specs:
            unique_specs[image_key] = spec
    
    unique_test_specs = list(unique_specs.values())
    print(f"Optimized: Building {len(unique_test_specs)} unique repo-version images for {len(test_specs)} instances")
    
    if force_rebuild:
        for spec in unique_test_specs:
    ...
    if len(env_failed) > 0:
        ...
        dont_run_specs = [spec for spec in unique_test_specs if spec.env_image_key in env_failed]
        unique_test_specs = [spec for spec in unique_test_specs if spec.env_image_key not in env_failed]
        print(f"Skipping {len(dont_run_specs)} unique images - due to failed env image builds")
    print(f"Building {len(unique_test_specs)} unique instance images")
    successful, failed = list(), list()

    # Build the instance images (using deduplicated unique specs)
    with tqdm(
        total=len(unique_test_specs), smoothing=0, desc="Building unique instance images"
    ) as pbar:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Create a future for each unique image to build
            futures = {
                executor.submit(
                ...
                for test_spec in unique_test_specs
            }
...
def build_container(
    ...
    Args:
        ...
        is_large (bool): Whether the instance is large, which may require more resources
    """
    # Build corresponding instance image
    if force_rebuild and not from_remote:
        remove_image(client, test_spec.instance_image_key, "quiet")
    elif force_rebuild:
        raise NotImplementedError("Can't remove image and from remote at the same time.")
    if not from_remote:
    ...
    try:
        ...
        # Reduce CPU allocation to work with server's 4 CPU limit
        # Use 0.5 CPU per container instead of 2 CPUs
        nano_cpus = config.get("nano_cpus", int(0.5e9))  # 0.5 CPU instead of 2 CPUs
        if is_large:
            nano_cpus = 2 * nano_cpus  # Reduce from 5x to 2x for large instances
