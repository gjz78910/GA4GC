...
from collect.utils import (
    ...
    Repo,
    get_inference_time_from_actions,
)
...
def main(pr_file: str, output: str, token: Optional[str] = None, cutoff_date: str = None):
    """
    ...
    def load_repo(repo_name):
        ...
        # return Repo(owner, repo, token=token, cutoff_date=cutoff_date, need_workflow=True)
        return Repo(owner, repo, token=token, cutoff_date=cutoff_date, need_workflow=False)
        ...
        with open(all_output) as f:
            for line in f:
                ...
                if is_valid_instance(pr):
                    ...
                    # if type(pr["base_actions_duration"]) == dict and type(pr["head_actions_duration"]) == dict:
                    #     with_tests += 1
        ...
        with open(output, write_mode) as output:
            for ix, line in enumerate(open(pr_file)):
                ...
                if is_valid_instance(instance):
                    ...
                    # if type(instance["base_actions_duration"]) == dict and type(instance["head_actions_duration"]) == dict:
