...
def run_instance(
    ...
    try:
        ...
        profile_test = Path(log_dir / "profile_test.py")
        with open(Path("harness") / "profile_test.py", 'r', encoding='utf-8') as file:
            content = file.read()
        profile_test.write_text(content)
        logger.info(
            f"Profile_test script for {commit_id} written to {profile_test}; copying to container..."
        )
        copy_to_container(container, profile_test, Path("/testbed/profile_test.py"))

        eval_file = Path(log_dir / "eval.sh")
        logger.info(f"Eval script coverage {test_spec.eval_script_function}")
        eval_file.write_text(test_spec.eval_script_function)
        logger.info(
        ...
        for idx, _ in enumerate(test_spec.efficiency_test):
            local_path = log_dir / f"function{idx}.json"
            container_path = f"/testbed/function{idx}.json"
            try:
                copy_from_container(container, container_path, local_path)
            except:
                logger.info(f"Can't find function{idx}.json")
                continue
        ...
        for idx, test in enumerate(test_spec.efficiency_test):
            function_path = log_dir / f"function{idx}.json"
            ...
            for func in function_info:
                unique_function.add(json.dumps({"mod":func["mod"], "func_name": func["func_name"]}))
        unique_function = [json.loads(func) for func in unique_function]
        logger.info(
            f"Functions: {functions}\n"
            f"Unique function: {unique_function}"
        )

        # Write report to report.json
        with open(report_path, "w") as f:
            f.write(json.dumps({"functions":functions, "function_total": unique_function}, indent=4))
        return commit_id, functions
