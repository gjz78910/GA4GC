...
from harness.test_spec import make_test_spec, TestSpec, REPEAT_TIME
...
def run_instance(
    ...
    try:
        ...
        container = build_container(test_spec, client, run_id, logger, rm_image, force_rebuild, is_large = True)
        container.start()
        ...
        # Run base eval script, write output to logs
        ...
        os.makedirs(log_dir / "report_base", exist_ok=True)
        test_output_path = log_dir / "test_output_base.txt"
        ...
        for re_idx in range(REPEAT_TIME):
            logger.info(f"Copying report from container for {instance_id} ({re_idx + 1}/{REPEAT_TIME})")
            # Copy report from container to local
            local_path = log_dir / "report_base" / f"report{re_idx}.json"
        ...
        for idx in range(REPEAT_TIME):
            base_path = log_dir / f"report_base/report{idx}.json"
            human_path = log_dir / f"report/report{idx}.json"
            if base_path.exists():
...
def get_dataset_from_preds(
    ...
    for instance in dataset:
        ...
        #     / run_id
        #     / prediction["model_name_or_path"].replace("/", "__")
        #     / prediction[KEY_INSTANCE_ID] / "report"
        #     / "report9.json"
        # )
        # report2_file = (
        #     RUN_EVALUATION_LOG_DIR
        #     / run_id
        #     / prediction["model_name_or_path"].replace("/", "__")
        #     / prediction[KEY_INSTANCE_ID] / "report_base"
        #     / "report9.json"
        # )
        if report_file.exists():# and report2_file.exists():
...
def get_gold_predictions(dataset):
    """
...
def main(
    ...
    full_dataset = load_sweperf_dataset(dataset_name, split, instance_ids)

    # load predictions as map of instance_id to prediction
    predictions = get_gold_predictions(full_dataset)
    predictions = {pred[KEY_INSTANCE_ID]: pred for pred in predictions}

    # get dataset from predictions
    dataset = get_dataset_from_preds(full_dataset, instance_ids, predictions, run_id)
    # build_env_images(client, full_dataset, force_rebuild, max_workers)
    # run_instances(predictions, dataset, cache_level, clean, force_rebuild, max_workers, run_id, timeout)
    # raise ValueError("Finish build docker images.")
