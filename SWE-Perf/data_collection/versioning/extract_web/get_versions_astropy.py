...
for task in data_tasks:
    ...
    for t in version_to_time:
        ...
        if t[1] < created_at:
            task["version"] = t[0]
            task["version_head"] = t[0]
            found = True
            break
    if not found:
        task["version"] = version_to_time[-1][0]
        task["version_head"] = version_to_time[-1][0]
...
os.makedirs(PATH_VERSION_ASTROPY,exist_ok=True)
with open(
    os.path.join(PATH_VERSION_ASTROPY, "astropy-task-instances_versions.json"),
    "w",
) as f:
